...
def test_get_snapshot_timeline_uses_chroma_primary(monkeypatch):
    service = VectorDBService()
    collection = _FakeCollection()
    ...
    assert [item["id"] for item in results] == ["newer", "older"]
    assert collection.get_calls == 1


def test_get_snapshot_timeline_returns_chroma_when_azure_not_set(monkeypatch):
    service = VectorDBService()
    collection = _FakeCollection()
